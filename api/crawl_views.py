...
from celery_tasks.crawl.tasks import sync_get_content_celery
...
def get_content_celery_view(request):
    # 爬内容了 自用
    res = n_data()
    try:
        # 校验
    ...
    try:
        if book1.using:
            ...
            book1.using = True
            book1.save()
        sync_get_content_celery.delay(book_id=book_id, current_chapter=book1.current, target=target, prefix=prefix)
